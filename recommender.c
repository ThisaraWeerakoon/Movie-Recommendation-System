...
static void recommender_compute(
	int userid,
	int No_of_users,
	...
	const double *newuser = &utility_matrix[(userid - 1) * No_of_movies]; // rating vector for this user (already in matrix)
		...
		for(i=0;i<1000;i++){ //selecting top 10 movies from the recommended movies
			...
			if(i>0 && (i+1)%10==0){
				...
				if (choice == 0)break;
					...
					/* continue showing recommendations */
				}
				else{
					printf("\nInvalid choice.\n\n");
					break;
...
void recommender(int uid){
	...
	get_utility_matrix(utility_matrix,"Dataset/ratings_learn.csv",No_of_movies, No_of_users, uid);
	...
	recommender_compute(uid, No_of_users, utility_matrix, normalized_matrix, 0, movienames, moviegenres);
...
void recommender_quiet(int uid){
	int No_of_users = findusers();
	double *utility_matrix = (double *)calloc((size_t)No_of_users * (size_t)No_of_movies, sizeof(double));
	...
	recommender_compute(uid, No_of_users, utility_matrix, normalized_matrix, 1, NULL, NULL);
